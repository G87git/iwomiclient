...
export default function Fichier() {
 ...
 // const { getFieldDecorator } = form;
    ...
    const columns = [
      ...
      { Header: "Nom Client", accessor: "nomcl" },
      { Header: "Reference", accessor: "refreg" },
      { Header: "Type Reg", accessor: "typer" },
      { 
        Header: "Devise", accessor: "dev", key:"dev", disableFilters: true,
         Cell:(dev) => state.profiles ? state.profiles[dev] : dev 
  
        //state.profiles ? state.profiles[prfle] : prfle || prfle,
     },

      
      { Header: "Montant", accessor: "mntreg" },
      { Header: "Montant en XAF", accessor: "mntregx" },
     ...
     const body = {
      ...
      return (
        <>
           ...
           <h2 className="text-lg font-bold"  style={{ cursor: "pointer" }} onClick={() => router.back()}><ArrowLeftOutlined/> &nbsp;Détails du fichier </h2>
